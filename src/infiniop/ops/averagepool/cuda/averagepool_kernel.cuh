...
__global__ void avgpool1d_pytorch_compatible_kernel(
  ...
  if (batch_idx >= batch_size || channel_idx >= channels ||
      output_idx >= output_length) {
    return;
  }

  // 计算输入和输出的偏移
  const T *input_ptr =
      input + batch_idx * channels * input_length + channel_idx * input_length;
  T *output_ptr = output + batch_idx * channels * output_length +
...
__global__ void avgpool2d_pytorch_compatible_kernel(
  ...
  if (batch_idx >= batch_size || channel_idx >= channels ||
      output_idx >= total_output_elements) {
    return;
  }
  ...
  T *output_ptr = output + batch_idx * channels * output_height * output_width +
  ...
  for (int kh = 0; kh < kernel_h; ++kh) {
    for (int kw = 0; kw < kernel_w; ++kw) {
      ...
      if (input_h >= 0 && input_h < input_height && input_w >= 0 &&
        ...
        valid_count++;
      } else if (input_h >= -pad_h && input_h < input_height + pad_h &&
                 input_w >= -pad_w && input_w < input_width + pad_w) {
        // 显式填充区域，值为0，只增加计数
        valid_count++;
      }
      // 其他位置是隐式填充，不计入分母
...
__global__ void avgpool3d_pytorch_compatible_kernel(
  ...
  const T *input_ptr = input + batch_idx * channels * input_spatial_size +
                       channel_idx * input_spatial_size;
  T *output_ptr = output + batch_idx * channels * output_spatial_size +
                  channel_idx * output_spatial_size;
  ...
  for (int kd = 0; kd < kernel_d; ++kd) {
    for (int kh = 0; kh < kernel_h; ++kh) {
      for (int kw = 0; kw < kernel_w; ++kw) {
        ...
        if (input_d >= 0 && input_d < input_depth && input_h >= 0 &&
            input_h < input_height && input_w >= 0 && input_w < input_width) {
          // 有效的输入位置，转换为单精度进行累加
          int input_idx =
              (input_d * input_height + input_h) * input_width + input_w;
          sum += static_cast<float>(input_ptr[input_idx]);
          valid_count++;
        } else if (input_d >= -pad_d && input_d < input_depth + pad_d &&
                   input_h >= -pad_h && input_h < input_height + pad_h &&
                   input_w >= -pad_w && input_w < input_width + pad_w) {
          // 显式填充区域，值为0，只增加计数
          valid_count++;
        }
...
// template <typename T>
// __global__ void avgpool1d_pytorch_compatible_kernel(
//     const T *input, T *output, int batch_size, int channels, int
//     input_length, int output_length, int kernel_size, int stride, int
//     padding) {
...
//   // 计算输入和输出的偏移
//   const T *input_ptr =
//       input + batch_idx * channels * input_length + channel_idx *
//       input_length;
//   T *output_ptr = output + batch_idx * channels * output_length +
//                   channel_idx * output_length;
...
//     if (input_pos >= 0 && input_pos < input_length) {
//       // 有效的输入位置 [0, input_length)
//       sum += input_ptr[input_pos];
//       valid_count++;
//     } else if (input_pos >= -padding && input_pos < input_length + padding) {
//       // 显式填充区域 [-padding, 0) 和 [input_length, input_length + padding)
//       valid_count++;
//     }
//     // 其他位置是隐式填充，不计入分母
//   }
...
//   // 计算平均值
//   if (valid_count > 0) {
//     output_ptr[output_idx] = sum / static_cast<T>(valid_count);
//   } else {
//     output_ptr[output_idx] = T(0);
//   }
// }
...
//     kernel_h, int kernel_w, int stride_h, int stride_w, int pad_h, int pad_w)
//     {
...
//                        batch_idx * channels * input_height * input_width +
//                        channel_idx * input_height * input_width;
//   T *output_ptr = output + batch_idx * channels * output_height *
//   output_width +
//                   channel_idx * output_height * output_width;
