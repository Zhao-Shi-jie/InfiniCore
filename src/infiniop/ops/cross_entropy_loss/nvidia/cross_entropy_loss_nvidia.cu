...
__global__ void softmaxCrossEntropy_per_sample(
    T_out* __restrict__ loss,                 
    ...
    int64_t ignore_index)
{
    ...
    if (ignore_index != LLONG_MIN && t == ignore_index) { loss[idx] = (T_out)0; return; }
    if (t < 0 || t >= C)                                { loss[idx] = (T_out)0; return; }
    ...
    const void *logits, const void *target, void *stream) const
{
    ...
    if (_dtype == INFINI_DTYPE_F32) {
        cuda::softmaxCrossEntropy_per_sample<float, float>
            <<<grid, block, 0, st>>>(per_sample, (const float*)logits, tgt_i64, N, C, inner, ignore_index);
    } else if (_dtype == INFINI_DTYPE_F16) {
        cuda::softmaxCrossEntropy_per_sample<half, float>
            <<<grid, block, 0, st>>>(per_sample, (const half*)logits, tgt_i64, N, C, inner, ignore_index);
    } else if (_dtype == INFINI_DTYPE_BF16) {
        cuda::softmaxCrossEntropy_per_sample<__nv_bfloat16, float>
            <<<grid, block, 0, st>>>(per_sample, (const __nv_bfloat16*)logits, tgt_i64, N, C, inner, ignore_index);
    }
    {
    ...
    if (cudaMemcpyAsync(h_loss.data(), per_sample, need_ws, cudaMemcpyDeviceToHost, st) != cudaSuccess)
        return INFINI_STATUS_INTERNAL_ERROR;
    if (cudaMemcpyAsync(h_tgt.data(),  tgt_i64,   (size_t)total * sizeof(int64_t), cudaMemcpyDeviceToHost, st) != cudaSuccess)
        return INFINI_STATUS_INTERNAL_ERROR;
    ...
    for (long long i = 0; i < total; ++i) {
        if (h_tgt[i] != ignore_index) { acc += (double)h_loss[i]; ++cnt; }
    }
    ...
    if (_dtype == INFINI_DTYPE_F32) {
        ...
        if (cudaMemcpyAsync(loss, &v, sizeof(__nv_bfloat16), cudaMemcpyHostToDevice, st) != cudaSuccess)
            return INFINI_STATUS_INTERNAL_ERROR;
    }
    if (cudaStreamSynchronize(st) != cudaSuccess)
        return INFINI_STATUS_INTERNAL_ERROR;
