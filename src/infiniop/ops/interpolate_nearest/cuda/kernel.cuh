...
compute_input_index_1d(size_t idx, const InterpolateNearestInfo &info) {
  ...
  float inv_scale = static_cast<float>(info.input_size[0]) /
                    static_cast<float>(info.output_size[0]);
  size_t input_w =
      min(static_cast<size_t>(floorf(static_cast<float>(w) * inv_scale)),
          info.input_size[0] - 1);

  return b * info.input_stride[0] + c * info.input_stride[1] +
         input_w * info.input_stride[2];
}
...
compute_input_index_2d(size_t idx, const InterpolateNearestInfo &info) {
  ...
  float inv_scale_h = static_cast<float>(info.input_size[0]) /
                      static_cast<float>(info.output_size[0]);
  float inv_scale_w = static_cast<float>(info.input_size[1]) /
                      static_cast<float>(info.output_size[1]);

  size_t input_h =
      min(static_cast<size_t>(floorf(static_cast<float>(h) * inv_scale_h)),
          info.input_size[0] - 1);
  size_t input_w =
      min(static_cast<size_t>(floorf(static_cast<float>(w) * inv_scale_w)),
          info.input_size[1] - 1);

  return b * info.input_stride[0] + c * info.input_stride[1] +
         input_h * info.input_stride[2] + input_w * info.input_stride[3];
}
...
compute_input_index_3d(size_t idx, const InterpolateNearestInfo &info) {
  ...
  float inv_scale_d = static_cast<float>(info.input_size[0]) /
                      static_cast<float>(info.output_size[0]);
  float inv_scale_h = static_cast<float>(info.input_size[1]) /
                      static_cast<float>(info.output_size[1]);
  float inv_scale_w = static_cast<float>(info.input_size[2]) /
                      static_cast<float>(info.output_size[2]);

  size_t input_d =
      min(static_cast<size_t>(floorf(static_cast<float>(d) * inv_scale_d)),
          info.input_size[0] - 1);
  size_t input_h =
      min(static_cast<size_t>(floorf(static_cast<float>(h) * inv_scale_h)),
          info.input_size[1] - 1);
  ...
  return b * info.input_stride[0] + c * info.input_stride[1] +
         input_d * info.input_stride[2] + input_h * info.input_stride[3] +
         input_w * info.input_stride[4];
}
...
compute_output_index(size_t idx, const InterpolateNearestInfo &info) {
  ...
  case INTERPOLATE_1D: {
    ...
    return b * info.output_stride[0] + c * info.output_stride[1] +
           w * info.output_stride[2];
  }
  ...
  case INTERPOLATE_3D: {
    ...
    b = temp / info.channels;
    return b * info.output_stride[0] + c * info.output_stride[1] +
           d * info.output_stride[2] + h * info.output_stride[3] +
           w * info.output_stride[4];
  }
...
calculate_total_elements(const InterpolateNearestInfo &info) {
  ...
  case INTERPOLATE_2D:
    ...
    break;
  case INTERPOLATE_3D:
    total *= info.output_size[0] * info.output_size[1] *
             info.output_size[2]; // depth * height * width
    break;
  }
  return total;
}
