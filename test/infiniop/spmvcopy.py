...
def spmv_pytorch_reference(values, row_ptr, col_indices, x, num_rows, num_cols):
    ...
    Only available on NVIDIA GPU devices.
    """
...
def is_nvidia_device(device_enum):
    """Check if the device is NVIDIA GPU"""
    return device_enum == InfiniDeviceEnum.NVIDIA
...
def test(
    ...
    # Only compute PyTorch sparse reference on NVIDIA devices
    y_pytorch_ref = None
    if device_enum and is_nvidia_device(device_enum):
        try:
            ...
            print("PyTorch sparse reference computed and verified")
        except Exception as e:
            print(f"Warning: PyTorch sparse reference failed: {e}")
            y_pytorch_ref = None
    else:
        print("Skipping PyTorch sparse reference (not NVIDIA device)")
    ...
    # Also check against PyTorch sparse reference if available
    if y_pytorch_ref is not None:
