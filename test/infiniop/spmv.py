...
_BASE_TEST_CASES = [
    # alpha, beta, rows, cols, crow, col
    (1.0, 0.0, 3, 4, [0, 2, 3, 5], [0, 2, 1, 0, 3]),
    (0.5, 1.0, 4, 5, [0, 1, 1, 3, 4], [2, 0, 4, 1]),
]
...
def test(
    ...
    x = TestTensor((cols,), None, dtype, device)
    y = TestTensor((rows,), None, dtype, device, mode="ones")
    ans = TestTensor((rows,), None, dtype, device, mode="zeros")
    ...
    ans.set_tensor(
        alpha * torch.matmul(sparse, x.torch_tensor()) + beta * y.torch_tensor()
    )
